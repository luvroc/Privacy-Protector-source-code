...
std::ostream& operator<<(std::ostream& os, const Keys::PublicKey& key) {
  os << "PublicKey{ g: " << key.g << ", y: " << key.y << " }";
  return os;
}

std::ostream& operator<<(std::ostream& os, const Keys::PrivateKey& key) {
  os << "PrivateKey{ x: " << key.x << " }";
  return os;
}
}  // namespace yacl::crypto
